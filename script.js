    ...
    if (learnMoreBtn && infoPanel) {
        learnMoreBtn.addEventListener('click', () => {
            ...
            if (infoPanel.classList.contains('visible')) {
                learnMoreBtn.innerText = 'Chiudi';
                setTimeout(() => {
                    infoPanel.scrollIntoView({ behavior: 'smooth', block: 'start' });
                }, 300);
            } else {
                learnMoreBtn.innerText = 'Scopri la Dimora';
            }
        });
    }
});
...
function selectTriviaOption(btn, isCorrect, feedbackId) {
    ...
    // Update feedback status (Italian)
    if (feedbackStatus) {
        feedbackStatus.innerText = isCorrect ? 'Esatto!' : 'Non proprio.';
